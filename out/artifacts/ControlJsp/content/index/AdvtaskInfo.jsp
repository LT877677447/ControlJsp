            ...
            <div class="am-u-sm-12 am-u-md-2">
                <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                        <button id="add_advtaskInfo" type="button" class="am-btn am-btn-default"><span class="am-icon-plus"></span> 新增</button>
                ...
                <form class="am-form">
                    <table class="am-table am-table-striped am-table-hover table-main">
                        ...
                        <tr>
                            ...
                            <th class="table-author">任务名</th>
                            <th class="table-author am-hide-sm-only">类型</th>
                            <th class="table-author am-hide-sm-only">APK_ID</th>
                            <th class="table-date am-hide-sm-only">起始日期</th>
                            <th class="table-date am-hide-sm-only">结束日期</th>
                            <th class="table-date am-hide-sm-only">新增时间</th>
                            <th class="table-date am-hide-sm-only">留存时间</th>
                            <th class="table-date am-hide-sm-only">新增时间线</th>
                            <th class="table-date am-hide-sm-only">留存时间线</th>
                            <th class="table-date am-hide-sm-only">留存规则</th>
                            <th class="table-date am-hide-sm-only">总量</th>
                            <th class="table-date am-hide-sm-only">打开次数</th>
                            <th class="table-date am-hide-sm-only">每日新增</th>
                            <th class="table-date am-hide-sm-only">已完成</th>
                            <th class="table-date am-hide-sm-only">状态</th>
                            <th class="table-date am-hide-sm-only">别名</th>
                            <th class="table-date am-hide-sm-only">操作</th>
                        </tr>
                        </thead>
                        <tbody id="content_table">
                        <c:forEach items="${AdvtaskInfoList}" var="rs" varStatus="stat">
                            <tr>
                                <td><input type="checkbox"/></td>
                                <td>${rs.adv_id}</td>
                                <td>${rs.name}</td>
                                <c:if test="${rs.adv_type == 1}">
                                    <td>普通cpa</td>
                                </c:if>
                                <c:if test="${rs.adv_type == 2}">
                                    <td>充值(2)</td>
                                </c:if>
                                <c:if test="${rs.adv_type == 3}">
                                    <td>充值(3)</td>
                                </c:if>
                                <c:if test="${rs.adv_type == 10}">
                                    <td>养粉|活跃</td>
                                </c:if>
                                <td class="am-hide-sm-only">${rs.apkid}</td>
                                <td class="am-hide-sm-only"><fmt:formatDate value="${rs.start_date}"
                                                                            pattern="yyyy年MM月dd日 HH时mm分ss秒"/></td>
                                <td class="am-hide-sm-only"><fmt:formatDate value="${rs.end_date}"
                                                                            pattern="yyyy年MM月dd日 HH时mm分ss秒"/></td>
                                <td class="am-hide-sm-only">${rs.requesttime}</td>
                                <td class="am-hide-sm-only">${rs.remaintime}</td>
                                <td class="am-hide-sm-only">${rs.timeline}</td>
                                <td class="am-hide-sm-only">${rs.rem_timeline}</td>
                                <td class="am-hide-sm-only">${rs.remain_rule}</td>
                                <td class="am-hide-sm-only">${rs.allcount}</td>
                                <td class="am-hide-sm-only">${rs.dayopencount}</td>
                                <td class="am-hide-sm-only">${rs.dayusercount}</td>
                                <td class="am-hide-sm-only">${rs.alldocount}</td>
                                <c:if test="${rs.onlineflag == 0}">
                                    <td class="am-hide-sm-only">停新增</td>
                                </c:if>
                                <c:if test="${rs.onlineflag == 1}">
                                    <td class="am-hide-sm-only">运行</td>
                                </c:if>
                                <c:if test="${rs.onlineflag == 2}">
                                    <td class="am-hide-sm-only">停止</td>
                                </c:if>
                                <td class="am-hide-sm-only">${rs.alias}</td>
                                    <%--<td id="operation">--%>
                                <td class="operation">
                                    <div class="am-btn-toolbar">
                                        <div class="am-btn-group am-btn-group-xs">
            ...
            <div class="am-modal-bd">
                <form class="am-form am-form-horizontal">
                    ...
                    <div class="am-form-group">
                        ...
                        <div class="am-u-md-8">
                            <%--<input id="modal_adv_type" type="text" class="am-modal-prompt-input">--%>
                            <select id="modal_add_adv_type" data-am-selected="{btnWidth:'50%',btnSize:'sm',maxHeight:250}">
                                <option value="1">普通cpa</option>
                                <option value="2">充值(2)</option>
                                <option value="3">充值(3)</option>
                                <option value="10">养粉|活跃</option>
...
<script>
    ...
    $(function () {
        $("#pageIndex").change(function () {
            ...
            $.post({
                url: '<c:url value="/AdvList/PageQuery"/>',
                data: param,
                contentType: "application/x-www-form-urlencoded;charset=UTF-8",
                dataType: "json",
                success: function (data, textStatus, jqXHR) {
                    var list = data.AdvtaskInfoList;
                    var pageModel = data.pageModel;
                    var apkinfo = data.advtaskInfo;
                    ...
                    $.each(list, function () {
                            ...
                            $("<td></td>").html(this.adv_id).appendTo(tr);
                            $("<td/>").html(this.name).appendTo(tr);
                            if (this.adv_type == 1) {
                                $("<td class='am-hide-sm-only'/>").html('普通cpa').appendTo(tr);
                            }
                            if (this.adv_type == 2) {
                                $("<td class='am-hide-sm-only'/>").html('充值(2)').appendTo(tr);
                            }
                            if (this.adv_type == 3) {
                                $("<td class='am-hide-sm-only'/>").html('充值(3)').appendTo(tr);
                            }
                            if (this.adv_type == 10) {
                                $("<td class='am-hide-sm-only'/>").html('养粉|活跃').appendTo(tr);
                            }
                            $("<td class='am-hide-sm-only'/>").html(this.apkid).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(startDate[index]).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(endDate[index]).appendTo(tr);
                            index++;
                            $("<td class='am-hide-sm-only'/>").html(this.requesttime).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.remaintime).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.timeline).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.rem_timeline).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.remain_rule).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.allcount).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.dayopencount).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.dayusercount).appendTo(tr);
                            $("<td class='am-hide-sm-only'/>").html(this.alldocount).appendTo(tr);
                            if (this.onlineflag == 0) {
                                $("<td class='am-hide-sm-only'/>").html('停新增').appendTo(tr);
                            }
                            if (this.onlineflag == 1) {
                                $("<td class='am-hide-sm-only'/>").html('运行').appendTo(tr);
                            }
                            if (this.onlineflag == 2) {
                                $("<td class='am-hide-sm-only'/>").html('停止').appendTo(tr);
                            }

                            $("<td class='am-hide-sm-only'/>").html(this.alias).appendTo(tr);
        ...
        $("#btn_search").on("click", function () {
            ...
            window.location.href = '<c:url value="/index/AdvList?"/>' + param;
        });
